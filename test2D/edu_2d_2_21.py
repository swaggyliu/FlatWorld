...
def edu_2d_2_15():
    ...
    init_sim()
    rigid1 = BoxRigid(2, [0.3 + 0.0025 * math.sqrt(3), 0.2025 + 0.1 * math.sqrt(3)], [0.4, 0.01], [-math.pi / 3], 1.0)
    rigid2 = BoxRigid(2, [0.1, 0.6], [0.2, 0.8], [0, 0], 1.0)
    rigid3 = BoxRigid(2, [0.6, 0.1], [0.8, 0.2], [0, 0], 1.0)
    bcs = [Gravity([0, -10.0])]
    rigiddomain1 = RigidBodyDomain(rigid1, bcs=[Gravity([0, -10.0])], friction=1)
    rigiddomain2 = RigidBodyDomain(rigid2, bcs=[FixedAll([0])], friction=0)
    rigiddomain3 = RigidBodyDomain(rigid3, bcs=[FixedAll([0])], friction=0.01)  ##greater than 0.28868
    ...
    while gui.running and t < 10.0:
        ...
        rigiddomain3.draw(gui, 0x000000, 720)
