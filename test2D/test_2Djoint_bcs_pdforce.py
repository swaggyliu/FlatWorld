...
def test_2Djoint_rotation(headless=False):
    ...
    joint1 = RevoluteJoint(0, 1, joint1_anchor, [0, 0], stiff=100, damping=3.0)

    # Joint 2: Revolute with angular acceleration (pi rad/s²)
    joint2_anchor = [0.7, 0.7]
    joint2 = RevoluteJoint(2, 3, joint2_anchor, [0, 0], stiff=100, damping=3.0)
...
def test_2Djoint_rotation_smallMass(headless=False):
    """Test 2D joints with rotation boundary conditions.
    ...
    # Setup 1: Revolute joint with constant angular velocity (like a motor)
    # Fixed anchor at center, body rotates at constant speed
    anchor1 = BallRigid(2, [0.3, 0.7], radius, 100.0)
    origin1 = [0.3, 0.85]
    body1 = BoxRigid(2, origin1, [0.04, 0.1], [0.0], 0.01)
    anchor1_domain = RigidBodyDomain(anchor1, [FixedAll([0])], considerContact=False)
    body1_domain = RigidBodyDomain(body1, [], considerContact=False)

    # Setup 2: Revolute joint with angular acceleration
    # Another fixed anchor with body that accelerates rotationally
    anchor2 = BallRigid(2, [0.7, 0.7], radius, 100.0)
    origin2 = [0.7, 0.85]
    body2 = BoxRigid(2, origin2, [0.04, 0.1], [0.0], 0.01)
    anchor2_domain = RigidBodyDomain(anchor2, [FixedAll([0])], considerContact=False)
    body2_domain = RigidBodyDomain(body2, [], considerContact=False)
    ...
    # Joint 1: Revolute with constant angular velocity (pi rad/s)
    joint1_anchor = [0.3, 0.7]
    joint1 = RevoluteJoint(0, 1, joint1_anchor, [0, 0], stiff=100, damping=3.0, effort_limit=30.0, velocity_limit=50.0)

    # Joint 2: Revolute with angular acceleration (pi rad/s²)
    joint2_anchor = [0.7, 0.7]
    joint2 = RevoluteJoint(2, 3, joint2_anchor, [0, 0], stiff=100, damping=3.0, effort_limit=30.0, velocity_limit=50.0)
...
def test_2Djoint_rotation_smallbutSame(headless=False):
    """Test 2D joints with rotation boundary conditions.
    ...
    # Setup 1: Revolute joint with constant angular velocity (like a motor)
    # Fixed anchor at center, body rotates at constant speed
    anchor1 = BallRigid(2, [0.3, 0.7], radius, 0.01)
    origin1 = [0.3, 0.85]
    body1 = BoxRigid(2, origin1, [0.04, 0.1], [0.0], 0.01)
    anchor1_domain = RigidBodyDomain(anchor1, [FixedAll([0])], considerContact=False)
    body1_domain = RigidBodyDomain(body1, [], considerContact=False)
    ...
    # Setup 2: Revolute joint with angular acceleration
    # Another fixed anchor with body that accelerates rotationally
    anchor2 = BallRigid(2, [0.7, 0.7], radius, 0.01)
    origin2 = [0.7, 0.85]
    body2 = BoxRigid(2, origin2, [0.04, 0.1], [0.0], 0.01)
    anchor2_domain = RigidBodyDomain(anchor2, [FixedAll([0])], considerContact=False)
    body2_domain = RigidBodyDomain(body2, [], considerContact=False)
...
if __name__ == "__main__":
    ...
    test_2Djoint_rotation_largemass(headless=args.headless)  # Not OK, not accurate enough
    # test_2Djoint_rotation_smallMass(headless=args.headless)  # collapse, but OK with smaller effort/velocity limits
    # test_2Djoint_rotation_smallbutSame(headless=args.headless)  # collapse, but OK with smaller effort/velocity limits
    """
    Meaning it is not due to mass ratio but absolute small mass that causes the issue. 
    This is likely due to numerical precision limits in the PD controller when dealing with very small masses.
    """
